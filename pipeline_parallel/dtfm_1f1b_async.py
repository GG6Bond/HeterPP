    ...
    def backward_micro_batch(self, backward_index, target_as_micro_batch=None, loss_func=None):
        if self.pp_rank == self.pipeline_group_size - 1:  # only send grad back to last node, do not receive
            with torch.cuda.stream(self.torch_comp_stream_b):
                ...
                loss.backward()
                self.torch_comp_stream_b.record_event(self.backward_comp_ready_events[backward_index])
            with torch.cuda.stream(self.torch_send_stream_b):
                cupy_backward_send_stream = cupy.cuda.ExternalStream(self.torch_send_stream_b.cuda_stream)
                self.torch_send_stream_b.wait_event(self.backward_comp_ready_events[backward_index])
            ...
            with torch.cuda.stream(self.torch_recv_stream_b):
                cupy_backward_recv_stream = cupy.cuda.ExternalStream(self.torch_recv_stream_b.cuda_stream)
                ...
                self.torch_recv_stream_b.record_event(self.backward_recv_ready_events[backward_index])
            with torch.cuda.stream(self.torch_comp_stream_b):
                self.torch_comp_stream_b.wait_event(self.backward_recv_ready_events[backward_index])
                self.profile_mark_backward_comp_start(backward_index)
